...
6. **Использование EDT MCP Server**:
   - `get_platform_documentation` — проверить существование метода/свойства платформы
   - `get_content_assist` — получить подсказки по типам и методам
   - `get_metadata_details` — проверить реквизиты и методы объектов метаданных
   - Проверяй имена переменных через `get_platform_documentation`, чтобы они не совпадали с именами глобального контекста (Документы, Справочники, Пользователи, Регистры, Метаданные, Константы и др.)
...
5. При необходимости используй EDT MCP Server для проверки синтаксиса и поиска методов
