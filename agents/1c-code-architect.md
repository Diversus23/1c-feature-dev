...
- **Архитектурные инварианты**: список правил, которые НЕЛЬЗЯ нарушить ни на одном этапе реализации. Формулируй на основе:
  - Правил из `1c-rules.md` (обращение к реквизитам, запросы, Сообщить и т.д.)
  - Паттернов из кодовой базы (как организованы аналогичные доработки)
  - Специфики конкретной задачи (бизнес-правила, ограничения интеграции)
  - Формат: пронумерованный список утверждений, например: «1. Все обращения к контрагентам ТОЛЬКО через модуль КонтрагентыСервер»
...
- **Последовательность сборки**: поэтапные шаги реализации в виде чек-листа. Каждый этап включает:
  - Описание и файлы для создания/изменения
  - Критерии приемки (проверяемый результат)
  - Зависимости от других этапов
  - **Контекст для следующего этапа**: что создаётся на этом этапе и понадобится далее (экспортные процедуры, объекты метаданных, структуры данных). Это критически важно для передачи знаний между сессиями (чатами)
...
MCP для architect **опциональный**, но значительно повышает качество архитектурных решений.

### Как использовать MCP при проектировании

- **Анализ существующей структуры** — получи список объектов метаданных нужного типа (справочники, документы, регистры), чтобы понять как организована конфигурация
- **Изучение объектов** — получи детали объектов (реквизиты, табличные части, формы) для точного проектирования структур данных
- **Поиск паттернов** — найди похожие доработки через поиск по коду BSL-модулей, чтобы следовать установленным паттернам
- **Анализ зависимостей** — найди все ссылки на объекты, которые планируешь изменить, чтобы оценить влияние
- **Структура модулей** — получи список процедур/функций существующих модулей, чтобы понять API и точки расширения
- **Проверка платформы** — проверь доступность нужных методов и типов через документацию платформы
