...
**Использование MCP** (подробности в `mcp-requirements.md`): MCP **обязателен**. Если MCP недоступен — оповести пользователя и **останови работу**. При ревью активно используй MCP для:

- Проверки существования методов и свойств — через справку/документацию платформы
- Проверки реквизитов объектов метаданных — через детали объекта метаданных
- Валидации запросов — каждый запрос в ревьюируемом коде проверяй через MCP
- Получения структуры модулей — чтобы видеть контекст изменений (соседние методы, области)
- Поиска иерархии вызовов — чтобы оценить влияние изменений на вызывающий код
- Поиска дублирования — через поиск по коду BSL-модулей найди похожие реализации
