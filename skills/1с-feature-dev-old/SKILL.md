...
3. Резюмируй понимание
4. **КЛАССИФИЦИРУЙ ЗАДАЧУ**: спроси пользователя или определи сам:
   - Простая: 1-2 файла, очевидная реализация
   - Средняя: новая форма, отчёт, несколько компонентов
   - Сложная: интеграция, рефакторинг, системные изменения
5. Получи подтверждение от пользователя
...
- **Средняя задача** - 2 агента
- **Сложная задача** - 3 агента

Действия:
1. **ОБЯЗАТЕЛЬНО** запусти нужное количество агентов `1c-code-explorer` параллельно. Каждый агент должен:
...
**Простая задача**:
- Ты сам проектируешь архитектуру без агентов
- Спрашиваешь подтверждение у пользователя

**Средняя задача**:
- 2 агента `1c-code-architect`

**Сложная задача**:
- 2 агента `1c-code-architect`

**Действия**:
1. **ОБЯЗАТЕЛЬНО** запусти **2 агента** `1c-code-architect` параллельно с разным фокусом: минимальные изменения (наименьшее изменение, максимальное переиспользование), чистая архитектура (поддерживаемость, элегантные абстракции), прагматичный баланс (скорость + качество)
...
**Действия**:
1. **ОБЯЗАТЕЛЬНО** запусти **1 агента** `1c-code-reviewer` параллельно с фокусом: простота/DRY/элегантность, баги/функциональная корректность, соглашения проекта/абстракции
2. Выяви проблемы наивысшей серьезности, которые рекомендуешь исправить
