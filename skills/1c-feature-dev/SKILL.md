...
| `05-implementation.md` | 5 | Лог реализации: выполненные этапы, изменённые файлы |
...
Если пользователь просит продолжить ранее начатую задачу — найди папку в `.tasks/` и определи фазу по последнему артефакту:
...
- Есть `05-implementation.md` → продолжи с Фазы 6 (ревью)
- Есть `03-architecture.md` с `- [ ]` или `- [🔄]` → продолжи с Фазы 5 (реализация)
...
Прочитай все существующие артефакты для восстановления контекста.
...
5. Сохрани `05-implementation.md`: выполненные этапы, изменённые файлы, ключевые решения
...
2. **Ревью агентом** (средние, сложные, критичные): запусти `1c-code-reviewer` с путём к `03-architecture.md`
...
4. **Документация** (опционально):
   ...
   - Если секция существует и `documentation: true`:
     ...
     c. При согласии — запусти агента `1c-code-documenter`:
        - Передай пути к артефактам: `01-requirements.md`, `03-architecture.md`, `05-implementation.md`, `07-summary.md`
