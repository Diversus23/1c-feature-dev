...
**Цель**: понять, что нужно построить, и оценить масштаб
...
4. Создай директорию `.tasks/task-[feature-name]/` и список задач со всеми фазами
5. Сохрани в файл `.tasks/task-[feature-name]/requirements.md`:
   ...
   - Оценка сложности с обоснованием
...
2. Запусти агентов `1c-code-explorer` согласно своей стратегии:
   - Передай путь к требованиям: `requirements.md`
...
1. Запусти агентов `1c-code-architect` согласно сложности задачи:
   - Передай пути к артефактам: `requirements.md`, `phase2-exploration.md`, `phase3-clarifications.md`
...
**УСЛОВИЕ**: эта фаза выполняется **только для сложных и критичных задач**. Для простых и средних — пропусти и переходи сразу к представлению плана пользователю (шаг 4).

**Действия** (только для сложных/критичных):
1. Запусти агента `1c-code-architect` для ревью плана:
   - Передай пути к артефактам: `requirements.md`, `phase2-exploration.md`, `phase3-clarifications.md`, `phase4-architecture.md`
   - Фокус ревью: полнота, корректность, реалистичность, соответствие требованиям, зависимости этапов
2. Если агент находит проблемы:
   - Обнови план в `.tasks/task-[feature-name]/phase4-architecture.md`
   - **Повтори ревью** (но не более 2 итераций — после второй исправь оставшееся сам)
3. Сохрани результаты ревью в `.tasks/task-[feature-name]/phase5-plan-review.md`

**Для всех задач** (включая простые/средние):
...
2. Запусти **одного** агента `1c-code-writer` для реализации **всех этапов**:
   - Передай путь к плану
   - Укажи, что нужно реализовать все этапы последовательно
   - Напомни про критерии приемки каждого этапа
   - Агент должен отмечать этапы в плане: `- [ ]` → `- [🔄]` → `- [x]`
3. После завершения агента проверь результат:
   - Все этапы отмечены как выполненные?
   - Файлы созданы/изменены согласно плану?
4. Если есть невыполненные этапы или проблемы:
   - Запусти агента `1c-code-writer` повторно для доработки
   - Передай путь к плану и опиши, что осталось незавершённым
...
**Действия**:
1. Запусти **одного** агента `1c-code-reviewer`:
   - Передай путь к плану: `phase4-architecture.md`
   - Фокус: баги, соответствие плану, правила из `1c-rules.md`, читаемость, DRY
2. Сохрани результаты ревью в `.tasks/task-[feature-name]/phase7-code-review.md`
3. **Представь выявленные проблемы пользователю и спроси, что он хочет сделать**:
   - Исправить сейчас
   - Исправить позже (добавить в TODO/технический долг)
   - Продолжить как есть
4. **Если "исправить сейчас"**:
   ...
   - **Вернись к началу Phase 7** (повторное ревью, но не более 2 итераций)
5. **Если "исправить позже"**:
