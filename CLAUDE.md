...
Плагин для Claude Code, реализующий 8-фазный воркфлоу разработки доработок в существующих конфигурациях 1С:Предприятие. Адаптация [feature-dev](https://github.com/anthropics/claude-plugins-official/tree/main/plugins/feature-dev) от Anthropic для платформы 1С.
...
1. **Скиллы** (`skills/*/SKILL.md`) — команды, доступные пользователю:
   - `1c-feature-dev` — основной воркфлоу из 8 фаз (Проверка окружения -> Сбор требований -> Исследование -> Архитектура -> Валидация плана -> Реализация -> Ревью кода -> Итоги). Оркестрирует агентов и управляет переходами между фазами.
   ...
   - `1c-docs` — автономная генерация документации для кода 1С: техническая (для AI/нейросетей) и пользовательская. Работает независимо от основного воркфлоу.
...
2. **Агенты** (`agents/*.md`) — специализированные субагенты, запускаемые параллельно (обычно по 3) на ключевых фазах:
   ...
   - `1c-code-architect` (Phase 3, 4) — проектирование архитектуры и валидация плана
   ...
   - `1c-code-simplifier` (Phase 6) — упрощение кода, запускается reviewer-ом при необходимости
   - `1c-code-test-generator` (Phase 7, опционально) — генерация тестов YaXUnit и сценариев Vanessa Automation
   - `1c-code-documenter` (Phase 7, опционально / standalone) — генерация технической и пользовательской документации
...
- Phase 3 создаёт файл плана `03-architecture.md` с **архитектурными инвариантами** и этапами реализации — Phase 5 и 6 работают по этому плану
- Phase 4 требует явного одобрения пользователя для ВСЕХ задач (включая простые) перед реализацией
- Phase 5 создаёт **артефакт после каждого этапа** (`05-implementation-stage-NN.md`) с контекстом для следующего этапа — это обеспечивает бесшовное продолжение работы в новом чате
- Phase 5 выполняет **мини-валидацию (DoD)** после каждого этапа: критерии приемки + MCP-валидация + проверка инвариантов
- Phase 5 поддерживает **паузу между этапами** (`pause_between_stages: true` в CLAUDE.md целевого проекта) для ручной проверки промежуточных результатов
- Phase 6 итеративна: MCP-валидация + ревью -> исправление -> повторное ревью, пока нет критических проблем
- Агенты используют MCP по категориям возможностей, не привязываясь к конкретным именам tools (см. `mcp-requirements.md`)
- Phase 7 проверяет конфигурацию документации в `CLAUDE.md` целевого проекта и предлагает генерацию при `documentation: true`
- **Возобновление задачи** работает на уровне этапов: при старте нового чата система находит последний `05-implementation-stage-NN.md`, читает контекст и продолжает с незавершённого этапа
